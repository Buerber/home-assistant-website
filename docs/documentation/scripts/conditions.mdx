...
与触发器不同，触发器始终是`or`，条件默认是`and` - 所有条件都必须为真。
...
目前，你需要像这样格式化条件才能使用[自动化编辑器](https://www.home-assistant.io/docs/automation/editor/)编辑它们。
...
### NOT条件
在一个条件语句中测试多个条件。如果所有嵌入的条件都**不**为真，则通过。
...
你可以选择使用`value_template`来处理状态的值，然后再对其进行测试。
