...
恒温器卡片提供对[气候](https://www.home-assistant.io/integrations/#climate)实体的控制，允许你更改实体的温度和模式。
</p>

![恒温器卡片的截图。](./img/thermostat_1.png)
<p className='text-center font-extralight'>恒温器卡片的截图。</p>
...
#### 配置变量  
    <div>
        ...
        <p className="text-sm text-gray-400 m-0" style={{margin:'0'}}>`thermostat`</p>
        <Separator className="my-4" />
    </div>

    <div>
        <p className="m-0 pb-2" style={{margin:'0'}}> entity <span className="text-xs text-red-400">字符串 必填</span></p>
        <p className="text-sm text-gray-400 m-0" style={{margin:'0'}}>`climate`域的实体ID。</p>
        <Separator className="my-4" />
    </div>
    ...
    <div>
        <p className="m-0 pb-2" style={{margin:'0'}}> show_current_as_primary <span className="text-xs text-gray-400">布尔值 (可选，默认：false)</span></p>
        <p className="text-sm text-gray-400 m-0" style={{margin:'0'}}>显示当前温度作为主要信息，而不是目标温度。目标温度将作为次要信息显示。</p>
        <Separator className="my-4" />
    </div>
