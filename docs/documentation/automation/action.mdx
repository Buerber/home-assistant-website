...
<p className="text-xl font-semibold">自动化规则的动作是指规则触发时执行的动作。动作部分遵循[脚本语法](https://www.home-assistant.io/docs/scripts/)，可通过其他操作或事件与任何内容进行交互。</p>

对于动作，您可以指定它应该应用的 `entity_id`（实体ID）以及可选参数（例如指定亮度）。

您还可以执行激活[场景](https://www.home-assistant.io/integrations/scene/)的操作，这将允许您定义设备的预期状态，然后由Home Assistant执行相应的动作。
