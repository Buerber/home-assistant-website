...
<p className="text-xl font-semibold"> 触发器是启动自动化规则处理的条件。当自动化的任何触发器变为 `true`（触发器被触发）时，Home Assistant将验证[条件](/docs/documentation/automation/condition)（如果有），并调用[动作](/docs/documentation/automation/action)。</p>

自动化可以由事件、特定实体状态、特定时间等触发。这些可以直接指定或通过模板更灵活地指定。也可以为一个自动化指定多个触发器。

- [触发器ID](/docs/documentation/automation/trigger#触发器ID)
- [触发器变量](/docs/documentation/automation/trigger#触发器变量)
- [事件触发器](/docs/documentation/automation/trigger#事件触发器)
- [HomeAssistant触发器](/docs/documentation/automation/trigger#HomeAssistant触发器)
- [MQTT触发器](/docs/documentation/automation/trigger#MQTT触发器)
- [数值状态触发器](/docs/documentation/automation/trigger#数值状态触发器)
- [状态触发器](/docs/documentation/automation/trigger#状态触发器)
- [太阳触发器](/docs/documentation/automation/trigger#太阳触发器)
- [标签触发器](/docs/documentation/automation/trigger#标签触发器)
- [模板触发器](/docs/documentation/automation/trigger#模板触发器)
- [时间触发器](/docs/documentation/automation/trigger#时间触发器)
- [时间模式触发器](/docs/documentation/automation/trigger#时间模式触发器)
- [持久通知触发器](/docs/documentation/automation/trigger#持久通知触发器)
- [Webhook触发器](/docs/documentation/automation/trigger#Webhook触发器)
- [区域触发器](/docs/documentation/automation/trigger#区域触发器)
- [地理位置触发器](/docs/documentation/automation/trigger#地理位置触发器)
- [设备触发器](/docs/documentation/automation/trigger#设备触发器)
- [日历触发器](/docs/documentation/automation/trigger#日历触发器)
- [句子触发器](/docs/documentation/automation/trigger#句子触发器)
- [多个触发器](/docs/documentation/automation/trigger#多个触发器)
- [同一触发器的多个实体ID](/docs/documentation/automation/trigger#同一触发器的多个实体ID)
- [禁用触发器](/docs/documentation/automation/trigger#禁用触发器)
- [合并触发器列表](/docs/documentation/automation/trigger#合并触发器列表)
...
当在给定的 MQTT 主题上接收到特定消息时触发。可以选择匹配通过主题发送的负载。默认的负载编码是 `'utf-8'`。对于图像和其他字节负载，使用 `encoding: ''` 完全禁用负载解码。
...
:::note
 `topic` 和 `payload` 模板仅在设置触发器时计算/执行，它们不会为每个传入的MQTT消息重新计算/执行。
:::
...
:::note
同时列出 `above` 和 `below` 意味着 `numeric_state` 必须在两个值之间。在上面的示例中，如果 `numeric_state` 进入 17.1-24.9 范围（高于17且低于25），触发器将触发一次。只有在离开定义的范围并再次进入后，它才会再次触发。
:::
...
通常，当任何给定实体的状态 `changes` 时，状态触发器会触发。行为如下：
...
模板触发器通过在识别的任何实体改变状态时计算/执行[模板](https://www.home-assistant.io/docs/configuration/templating/)来工作。如果状态变化导致模板渲染为'true'（非零数字或任何字符串`true`、`yes`、`on`、`enable`）而之前为'false'（其他任何内容），触发器将触发。
...
当`value_template`变为'true'时，将计算/执行`for`模板。
